...
def run_lstm_model(stockcode):
    def LSTMtest(data):
        ...
        keep_prob = tf.placeholder(tf.float32, name='keep_prob')  # dropout prevents overfitting
            ...
            with tf.Session() as sess:
                ...
                for i in range(200):
                    for step in range(len(batch_index) - 1):
                        ...
                        state_, loss_ = sess.run([train_op, loss], feed_dict={X: train_x[batch_index[step]:batch_index[step + 1]], Y: train_y[batch_index[step]:batch_index[step + 1]], keep_prob: 0.5})
                    print("Number of iterations:", i, " loss:", loss_)
                    theloss.append(loss_)
                
                isExists=os.path.exists(str(stockcode)+"//model//"+time)
                if not isExists:
                  os.makedirs(str(stockcode)+"//model//"+time)
    
                modelpath=str(stockcode)+"//model//"+time+"//model.ckpt"
